...
%  To do the bootstrap task and calculate the significance between two NN
clear all;
close all;
clc;
...
cdaddr=strcat('F:\Breast_Dataset\');
saveaddr='F:\Breast_Dataset\Buff_data\';
cd (cdaddr);
        
%%  training task
for data_Strategy = 3 %num_dataStrategy
...
%         testFeatures = activations(net, augimdsTest, layersTransfor);
        ...
        for flag_bootstrap = 1:num_bootstrap
            ...
            predictedLabels = classify(netTransfer,0,testFeatures.testFeatures);
...
save AUC_matrix_C_S1_Fuzzy_4c accuracy_matrix
...
%% Two sample T-test
clear all;
% load('AUC_matrix_C_S1_6c.mat')
% accuracy_matrix = squeeze(accuracy_matrix(1,1,:));
% AA = accuracy_matrix';
% 
% load('AUC_matrix_C_block1.mat')
% accuracy_matrix = squeeze(accuracy_matrix(1,3,:));
% BB = accuracy_matrix';
% 
% load('AUC_matrix_C_block2.mat')
% accuracy_matrix = squeeze(accuracy_matrix(2,3,:));
% CC = accuracy_matrix';
% 
% accuracy_matrix = [AA; BB; CC];
% save AUC_matrix_C accuracy_matrix

load('AUC_matrix_C.mat')
num_NN = size(accuracy_matrix,1)
BTN = 100;  %  the times of bootstrap
